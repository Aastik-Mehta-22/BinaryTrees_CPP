...
class Solution {
  public:
    void leftTraverse(Node* node,vector<int> &ans){
        
         while (node) {
        if (node->left || node->right) // Avoid adding leaf nodes here
            ans.push_back(node->data);
        if (node->left)
            node = node->left;
        else
            node = node->right; // Go right if no left child
        }
    }
   ...
   void rightTraverse(Node* node, vector<int> &ans) {
    ...
    while (node) {
        ...
        if (node->right)
            ...
            node = node->left; // Go left if no right child
    }
